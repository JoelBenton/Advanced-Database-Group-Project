...
import { findDocuments } from "./mongoHelpers";
import { findPatientMultipleConditions } from "./queries/patient";
...
export async function getPatientsOrStatement(Conditions: PatientQueryConditions = {}) {
  const data = await findDocuments("patient", findPatientMultipleConditions(Conditions));
  return data;
}
